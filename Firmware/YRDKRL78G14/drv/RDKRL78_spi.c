...
void SPI_Send(uint8_t aDevice, uint8_t *aData, uint16_t aLength)
{
    uint8_t noRXData[32];
    ...
    R_CSI21_Send_Receive(aData, aLength, &noRXData[0]);
    while(G_CSI21_SendingData);
    
    SPI_CS_End(aDevice);
}

uint8_t SPI_SendReceive(uint8_t aDevice, uint8_t *aTXData, uint16_t aTXLength, uint8_t *aRXData)
{
    uint16_t timeout;
	G_CSI21_SendingData = 1;
