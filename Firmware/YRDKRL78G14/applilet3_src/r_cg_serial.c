...
uint8_t * gp_csi31_rx_address;         /* csi31 receive buffer address */
uint16_t  g_csi31_rx_length;           /* csi31 receive data length */
uint16_t  g_csi31_rx_count;            /* csi31 receive data count */
uint8_t * gp_csi31_tx_address;         /* csi31 send buffer address */
uint16_t  g_csi31_send_length;         /* csi31 send data length */
uint16_t  g_csi31_tx_count;            /* csi31 send data count */
...
/***********************************************************************************************************************
* Function Name: R_CSI31_Create
* Description  : This function initializes the CSI31 module.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
void R_CSI31_Create(void)
{
    ST1 |= _0008_SAU_CH3_STOP_TRG_ON;    /* disable CSI31 */
    CSIMK31 = 1U;    /* disable INTCSI31 interrupt */
    CSIIF31 = 0U;    /* clear INTCSI31 interrupt flag */
    /* Set INTCSI31 low priority */
    CSIPR131 = 1U;
    CSIPR031 = 1U;
...
/***********************************************************************************************************************
* Function Name: R_CSI31_Start
* Description  : This function starts the CSI31 module operation.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
void R_CSI31_Start(void)
{
    CSIIF31 = 0U;    /* clear INTCSI31 interrupt flag */
    CSIMK31 = 0U;    /* enable INTCSI31 */
    SO1 &= ~_0800_SAU_CH3_CLOCK_OUTPUT_1;   /* CSI31 clock initial level */
    SO1 &= ~_0008_SAU_CH3_DATA_OUTPUT_1;           /* CSI31 SO initial level */
    SOE1 |= _0008_SAU_CH3_OUTPUT_ENABLE;           /* enable CSI31 output */
...
/***********************************************************************************************************************
* Function Name: R_CSI31_Stop
* Description  : This function stops the CSI31 module operation.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
void R_CSI31_Stop(void)
{
    ST1 |= _0008_SAU_CH3_STOP_TRG_ON;        /* disable CSI31 */
    SOE1 &= ~_0008_SAU_CH3_OUTPUT_ENABLE;    /* disable CSI31 output */
    CSIMK31 = 1U;    /* disable INTCSI31 interrupt */
    CSIIF31 = 0U;    /* clear INTCSI31 interrupt flag */
}

/***********************************************************************************************************************
* Function Name: R_CSI31_Send_Receive
* Description  : This function sends and receives CSI31 data.
* Arguments    : tx_buf -
*                    transfer buffer pointer
*                tx_num -
*                    buffer size
...
*                    MD_OK or MD_ARGERROR
***********************************************************************************************************************/
MD_STATUS R_CSI31_Send_Receive(uint8_t * const tx_buf, uint16_t tx_num, uint8_t * const rx_buf)
{
    MD_STATUS status = MD_OK;
    ...
    {
        g_csi31_tx_count = tx_num;        /* send data count */
        gp_csi31_tx_address = tx_buf;     /* send buffer pointer */
        gp_csi31_rx_address = rx_buf;     /* receive buffer pointer */
        CSIMK31 = 1U;                     /* disable INTCSI31 interrupt */
        SIO31 = *gp_csi31_tx_address;    /* started by writing data to SDR[7:0] */
        gp_csi31_tx_address++;
        g_csi31_tx_count--;
        CSIMK31 = 0U;                     /* enable INTCSI31 interrupt */
    }
